...
\end{itemize}
% still-to-do
%\item Proof of (2): let d be the gcd of a and |x|. then by construction, $|x| = db, a = dc$, and b,c are relatively prime. Then $|x| / d = db /d = b$, so we wish to show $|x^a| = b$. But $(x^{a})^{b} = x^{ab} = x^{(dc)(b)} = (x^{db})^{c} = (x^{|x|})^{c} = (1)^c = 1$; so $|x^a|$ divides $b$ (by proposition 3 -- "$x^m = x^n = 1 \implies x^gcd(m,n) = 1$). Also, $x^{a|x^a|} = (x^a)^{|x^a|} = 1$, so $|x|$ divides $x |x^a|$. Thus $|x| \divides 
\end{itemize}
